---
created: 2023-03-08T10:03:11 (UTC +08:00)
tags: [C# 索引器（Indexer）]
...
> ## Excerpt
> C# 索引器（Indexer）  索引器（Indexer） 允许一个对象可以像数组一样使用下标的方式来访问。 当您为类定义一个索引器时，该类的行为就会像一个 虚拟数组（virtual array） 一样。您可以使用数组访问运算符 [ ] 来访问该类的的成员。   语法 一维索引器的语法如下：  [mycode4 type='csharp'] element-type this[int index]  {    // ..
